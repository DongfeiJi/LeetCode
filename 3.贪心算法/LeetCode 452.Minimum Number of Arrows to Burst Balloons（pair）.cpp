...
bool cmp(const std::pair<int, int> &a, const std::pair<int ,int> &b) {
    return a.first < b.first;//用于无需考虑左端点相同时候的排序,（sort排序结构体也会用到，这里return <代表升序）
}
...
public:
    int findMinArrowShots(std::vector<std::pair<int, int> >& points) {
    	...
    	int shoot_end = points[0].second;//初始化弓箭数目，第一个维护射击区间，就是第一个气球的左右端点
    	for (int i = 1; i < points.size(); i++){//此处就是遍历所有气球，看最多可以射爆多少气球，更新射击空间
	    	if (points[i].first <= shoot_end){
	    		shoot_begin = points[i].first;//新气球左端点小于设计区间右端点，那么说明可以击穿，更新射击区间左端点
    			if (shoot_end > points[i].second){
			    	shoot_end = points[i].second;//新球右端点小于设计区间左端点，那么说明可以击穿，更新设计区间右端点
					 //设计区间维护其实就是在不断缩小射击区间
				}
	    	}
	    	else{//不满足情况时，多用一个弓箭
	    		...
	    		shoot_end = points[i].second;//初始化新的弓箭设计区域
	    	}
	    }
	    return shoot_num;
    }
